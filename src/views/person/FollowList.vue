<template>
  <div class="followers">
    <ul class="followers-list">
      <li class="followers-item" v-for="(item, index) in followerList" :key="index">
        <div class="left">
          <div class="left-image">
            <el-avatar
              :src="item.avatar.indexOf('null') > -1 ? defaultUrl : item.avatar"
              size="medium" class="avatar"
...
<!--         <div class="right" @click="focusUser(item, index)" v-if="isRealUser">
          <button
...
export default {
  ...
  methods: {
    ...
    // 关注或者取消关注
    focusUser(user, index) {
      let focusId = user.uid
      ...
      focusUser({
        ...
        if(res && res.data.status) {
          this.changeFocusStatus(user, focusId)
        }
      })
    },
    // 改变关注按钮的状态(关注或者已关注)
    changeFocusStatus(user, focus_id, index) {
      let count = 1
      for(let i = 0; i < this.followerList.length;i++) {
        if(this.followerList[i].uid === focus_id) {
          if(this.followerList[i].isFocus) {
            this.followerList[i].isFocus = false
            if(count <= 1) {
              this.$store.commit('downFocusNum')  // 每次取关或者关注，数量只需要加减一次就够了
            }
            count += 1
          } else {
            this.followerList[i].isFocus = true
            if(count <= 1) {
  ...
  created() {
    console.log(this.followerList, 'followerList')
  }
}
</script>
